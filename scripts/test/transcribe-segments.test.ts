...
test('a retranscribe request beyond the allowed future skew is ignored', async () => {
  const segment = segmentEvent(OTHER_KEY, 'logbook-31', SHA_A)
  const covered = transcriptEvent(OTHER_KEY, segment.id, 'old text', 2000)
  const { deps, recorded } = makeDeps({
    fetchSegments: async () => [segment],
    fetchTranscriptEvents: async () => [covered],
    fetchRetranscribeRequests: async () => [retranscribeRequest(producerKey, segment.id, 2801)],
  })

  const result = await runTranscribeSweep(deps, {
    nowSeconds: () => 2500,
    maxRequestFutureSkewSeconds: 300,
  })
...
test('an accepted future-dated request is satisfied by the resulting companion', async () => {
  const segment = segmentEvent(OTHER_KEY, 'logbook-31', SHA_A)
  const oldTranscript = transcriptEvent(OTHER_KEY, segment.id, 'old text', 2000)
  const request = retranscribeRequest(producerKey, segment.id, 2600)
  const { deps, recorded } = makeDeps({
    fetchSegments: async () => [segment],
    fetchTranscriptEvents: async () => [oldTranscript],
    fetchRetranscribeRequests: async () => [request],
  })
  const options = {
    nowSeconds: () => 2500,
    maxRequestFutureSkewSeconds: 300,
  ...
  const first = await runTranscribeSweep(deps, options)

  assert.equal(first.transcribed, 1)
  assert.equal(recorded.published[0].created_at, request.created_at)

  const satisfied = transcriptEvent(compassKey, segment.id, 'new text', recorded.published[0].created_at)
  const { deps: secondDeps, recorded: secondRecorded } = makeDeps({
    fetchSegments: async () => [segment],
    fetchTranscriptEvents: async () => [satisfied],
    fetchRetranscribeRequests: async () => [request],
  })
  const second = await runTranscribeSweep(secondDeps, options)

  assert.equal(second.missing, 0)
  assert.equal(secondRecorded.published.length, 0)
})
