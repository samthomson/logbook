...
test('Compass progress does not inherit producer-authored completion claims', () => {
  const next = withReleaseProgress(
    {
      episodeStatus: 'cutting',
      sections: [],
      release: { completed: ['feed', 'podstr', 'announcement'] },
      publishedRss: { feedUrl: 'https://producer.invalid/feed.xml' },
    },
    { completed: ['audio', 'chapters'] },
    false,
  )
  assert.deepEqual(next.release, { completed: ['audio', 'chapters'] })
  assert.equal(next.publishedRss, null)
})
