...
test('filesystem ledger resumes and migrates the legacy release ledger', () => {
  const root = `/tmp/logbook-release-ledger-legacy-${process.pid}-${Date.now()}`
  const state = { revision, completed: { artifacts: true, feed: true } as const, terminal: false }
  mkdirSync(root, { recursive: true })
  writeFileSync(`${root}/logbook-31-release.json`, JSON.stringify(state), 'utf-8')
  try {
    const ledger = new FileReleaseLedger(root, 'logbook-31')
    assert.deepEqual(ledger.load(), state)
    assert.equal(existsSync(`${root}/logbook-31-current-release-ledger.json`), true)
    assert.deepEqual(ledger.load(), state)
  } finally {
...
test('Compass-authored verified progress resumes external stages', async () => {
  ...
  await runReleaseStages({
    ledger: store.ledger,
    revision: resume,
    trustedProgressAuthor: 'c'.repeat(64),
...
test('producer-authored release progress cannot skip external gates', async () => {
  const forged = manifestRevision({
    id: 'f'.repeat(64),
    created_at: 301,
    pubkey: 'p'.repeat(64),
    tags: [['d', 'logbook-31']],
    content: JSON.stringify({
      episodeStatus: 'cutting',
      sections: [],
      release: { completed: ['feed', 'podstr', 'announcement'] },
    }),
  })
  const store = memoryLedger()
  const calls: string[] = []
