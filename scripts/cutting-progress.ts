...
import { COMPASS_PUBKEY, KINDS, RELAYS } from './config.ts'
...
export function withReleaseProgress<T extends CuttingManifest>(
  ...
  trustExistingProgress = true,
): T {
  const completed = mergeCompleted(
    trustExistingProgress ? manifest.release?.completed : undefined,
    patch.completed ?? [],
  )
  const failed = patch.failed === undefined
    ? (trustExistingProgress && typeof manifest.release?.failed === 'string' && STEP_SET.has(manifest.release.failed)
  ...
  return {
    ...
    lastFailure: patch.lastFailure === undefined
      ? (trustExistingProgress ? manifest.lastFailure : undefined)
      : patch.lastFailure,
    publishedRss: mergePublishedRss(
      trustExistingProgress ? manifest.publishedRss : undefined,
      patch.publishedRss ?? undefined,
    ),
...
export async function writeCuttingProgress<T extends CuttingManifest>(params: {
  ...
  }, params.event.pubkey.toLowerCase() === COMPASS_PUBKEY)
