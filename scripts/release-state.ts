...
export function manifestRevision(event: RevisionEvent): ManifestRevision {
  ...
  return {
    id: event.id,
    author: typeof event.pubkey === 'string' ? event.pubkey.toLowerCase() : null,
    createdAt,
    dTag,
...
export class FileReleaseLedger implements ReleaseLedger {
  ...
  constructor(root: string, issueId: string, lockId = 'current') {
    ...
    this.legacyPath = join(root, `${issueId}-release.json`)
  }
  ...
  load(): ReleaseLedgerState | null {
    ...
    const state = JSON.parse(readFileSync(this.legacyPath, 'utf-8')) as ReleaseLedgerState
    // Migrate before returning so a restart after this read continues from the
    // lock-specific path and never repeats an acknowledged external stage.
    this.save(state)
    return state
  }
...
export interface ReleaseStageRunner {
  ...
  revision: ManifestRevision
  /** Only this signer may attest resumable release progress in a manifest. */
  trustedProgressAuthor?: string
...
export async function runReleaseStages({
  ledger,
  revision,
  trustedProgressAuthor,
  current,
  stages,
}: ReleaseStageRunner): Promise<ReleaseLedgerState> {
  ...
  if (!state) {
    const mayResumeManifestProgress = revision.author !== null
      && revision.author === trustedProgressAuthor?.toLowerCase()
    state = {
      revision,
      completed: mayResumeManifestProgress
        ? seedCompletedStages(releaseCompletedOf(revision.content))
        : {},
      terminal: false,
    }
    ledger.save(state)
