...
import { parseVerifiedSegment, verifyNostrEvent, type VerifiedSegment } from './segment-security.ts'
...
export interface TranscribeSweepOptions {
  ...
  /** Injectable wall clock for deterministic request-skew handling. */
  nowSeconds?: () => number
  /** Requests farther into the future than this are ignored. */
  maxRequestFutureSkewSeconds?: number
}

export const DEFAULT_MAX_REQUEST_FUTURE_SKEW_SECONDS = 300
...
function requestedAt(
  ...
  for (const event of requests) {
    ...
    if (event.created_at > nowSeconds + maxFutureSkewSeconds) continue
    if (!event.tags.some(([key, value]) => key === 'e' && value === segment.id)) continue
    if (newest === null || event.created_at > newest) newest = event.created_at
  }
  return newest
...
export async function runTranscribeSweep(
  ...
  const maxFutureSkewSeconds = options.maxRequestFutureSkewSeconds
    ?? DEFAULT_MAX_REQUEST_FUTURE_SKEW_SECONDS
  if (!Number.isFinite(nowSeconds)) throw new Error('nowSeconds must return a finite timestamp')
  if (!Number.isFinite(maxFutureSkewSeconds) || maxFutureSkewSeconds < 0) {
    throw new Error('maxRequestFutureSkewSeconds must be a non-negative finite number')
  }
  ...
  const requests = verified.length && !options.retranscribeAll
    ...
    : []
  const missing = verified.flatMap(({ event, parsed }) => {
    if (options.retranscribeAll) return [{ event, parsed, requestAt: null }]
    const requestAt = requestedAt(event, requests, producers, nowSeconds, maxFutureSkewSeconds)
    const companionAt = trustedCompanionAt(event, transcripts)
    if (companionAt === null || (requestAt !== null && requestAt > companionAt)) {
      return [{ event, parsed, requestAt }]
    }
    return []
  })
  ...
  try {
    for (const { event, parsed, requestAt } of batch) {
      ...
      const signed = await dependencies.signEvent({
        ...
        // A tolerated future timestamp must be matched or the same request
        // would out-age this companion forever on subsequent sweeps.
        created_at: Math.max(nowSeconds, requestAt ?? nowSeconds),
        tags: [
          ['e', event.id, '', 'root'],
...
export function assertWhisperConfigured(modelPath: string): void {
  const probe = spawnSync('whisper-cli', ['--help'], {
    encoding: 'utf-8',
    timeout: 10_000,
    maxBuffer: 1024 * 1024,
  })
...
function ffmpegToWav(audioPath: string, wavPath: string): void {
  const result = spawnSync(
    ...
    { encoding: 'utf-8', timeout: 120_000, maxBuffer: 4 * 1024 * 1024 },
  )
...
function whisperTranscribe(modelPath: string): (audioPath: string, workDir: string) => { text: string; chunks: TranscriptChunk[] } {
  return (audioPath, workDir) => {
    ...
    // The sweep reuses this directory for a bounded batch. Clear the previous
    // segment's outputs so a successful-but-outputless tool invocation cannot
    // reuse another contributor's audio or transcript.
    ...
    const result = spawnSync(
      ...
      { encoding: 'utf-8', timeout: 10 * 60_000, maxBuffer: 4 * 1024 * 1024 },
    )
    if (result.status !== 0) {
