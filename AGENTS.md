...
- Two relay roles, two names, both required (no silent defaults):
  ...
  `COMPASS_PUBKEY` is this deployment's Compass identity and may be a staging
  key; every newsletter query, route, cache check, and signer check pins to it.
  `REAL_COMPASS_PUBKEY` is only a seed-tool deny-list and never a consumer
  fallback for an unset `COMPASS_PUBKEY`.
...
- [x] **ADMIN-01**: Producer mode comes only from the Compass-signed relay list. Initial enrollment uses the trusted seed tool; clients fail closed to Compass if the list is unavailable.
...
- A stitch run that fails hands the episode back: the worker republishes the
  ...
  worker can retry.
- A recording whose measured true peak is below −66 dBFS receives a clear
  microphone warning before publication. The operator may still review and
  publish it deliberately; downstream loudness normalization must not be
  described as recovering missing speech.
